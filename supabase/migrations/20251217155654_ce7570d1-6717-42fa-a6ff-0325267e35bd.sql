-- Add has_completed_diagnostic column to user_data_completeness
ALTER TABLE public.user_data_completeness 
ADD COLUMN IF NOT EXISTS has_completed_diagnostic boolean DEFAULT false;

-- Update the refresh_user_completeness function to check for diagnostic scores
...
BEGIN
  ...
  -- Check for personal vision (professional)
  SELECT EXISTS(
    SELECT 1 FROM personal_goals 
    WHERE profile_id = user_id 
    AND goal_type = 'professional'
  ...
  -- Calculate score (adjusted to include diagnostic - now 8 items)
  v_score := 0;
